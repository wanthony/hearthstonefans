...
(function() {
  HSFans.controller('CardListCtrl', [
    '$scope', 'CardAPI', '$filter', function($scope, CardAPI, $filter) {
      var QUALITY_INDEX_MAP, card, _i, _len, _ref;
      $scope.api = CardAPI;
      QUALITY_INDEX_MAP = {
        basic: 0,
        common: 1,
      ...
      $scope.rowClass = function(card) {
        return "row-" + (card.quality.toLowerCase());
      };
      $scope.cellClass = function(card) {
        return "cell-" + (card.quality.toLowerCase());
      };
      $scope.predicate = 'name';
      $scope.reverse = false;
      $scope.changeSorting = function(predicate) {
      ...
      $scope.countCards = function(cards, quality) {
        var count, _j, _len1;
        count = 0;
      ...
      $scope.basicCount = function() {
        var cards, filteredCards;
        cards = angular.copy($scope.cards);
        filteredCards = $filter('filter')(cards, $scope.cardNameFilter);
        return $scope.countCards(filteredCards, 'Basic');
      };
      $scope.commonCount = function() {
        var cards, filteredCards;
        cards = angular.copy($scope.cards);
        filteredCards = $filter('filter')(cards, $scope.cardNameFilter);
        return $scope.countCards(filteredCards, 'Common');
      };
      $scope.rareCount = function() {
        var cards, filteredCards;
        cards = angular.copy($scope.cards);
        filteredCards = $filter('filter')(cards, $scope.cardNameFilter);
        return $scope.countCards(filteredCards, 'Rare');
      };
      $scope.epicCount = function() {
        var cards, filteredCards;
        cards = angular.copy($scope.cards);
        filteredCards = $filter('filter')(cards, $scope.cardNameFilter);
        return $scope.countCards(filteredCards, 'Epic');
      };
      $scope.legendaryCount = function() {
        var cards, filteredCards;
        cards = angular.copy($scope.cards);
        filteredCards = $filter('filter')(cards, $scope.cardNameFilter);
        return $scope.countCards(filteredCards, 'Legendary');
      };
      return $scope.totalCount = function() {
        var cards;
        cards = angular.copy($scope.cards);
        return $filter('filter')(cards, $scope.cardNameFilter).length;
      };
    }
  ]);
